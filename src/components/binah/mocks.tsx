...
function Bar({ w = 50, style }: { w?: number; style?: CSSProperties }) {
  return <div className="mock-bar" style={{ ...({ "--w": `${w}%` } as CSSProperties), ...style }} />;
}
...
function Row({
  ...
  return (
    <div className={`mock-row ${highlight ? "highlight" : ""}`} style={style}>
      {children}
    </div>
  );
}
    ...
    <div className="mock">
      <WindowFrame title="binah.app / inbox" style={{ top: 0, left: 0, width: "64%", bottom: "25%" }}>
        <Row>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontWeight: 500 }}>María López</div>
            <div style={{ color: "var(--mute)", fontSize: 11 }}>WhatsApp · hace 2 min</div>
          </div>
          <span className="mock-pill ink">WA</span>
        </Row>
        <Row highlight>
          <div className="mock-avatar ink" />
          <div style={{ flex: 1 }}>
            <div style={{ fontWeight: 500 }}>Andrés Ruiz</div>
            <div style={{ opacity: 0.7, fontSize: 11 }}>Instagram DM · ahora</div>
          </div>
          <span className="mock-pill" style={{ background: "#fff", color: "#000", borderColor: "#fff" }}>IG</span>
        </Row>
        <Row>
          <div className="mock-avatar" />
          <div style={{ flex: 1 }}>
            <div style={{ fontWeight: 500 }}>compras@acme.co</div>
            <div style={{ color: "var(--mute)", fontSize: 11 }}>Email · hace 8 min</div>
          </div>
        ...
        <Row>
          <div className="mock-avatar" />
          <div style={{ flex: 1 }}>
            <div style={{ fontWeight: 500 }}>Lina Vega</div>
            <div style={{ color: "var(--mute)", fontSize: 11 }}>WhatsApp · hace 15 min</div>
          </div>
          <span className="mock-pill">WA</span>
        </Row>
      </WindowFrame>
      ...
      <WindowFrame title="agente · reservas" style={{ bottom: 0, left: 0, width: "36%", height: "22%" }}>
        <div style={{ display: "flex", gap: 6 }}>
          {[1, 2, 3, 4, 5, 6, 7].map((i) => (
            <div key={i} style={{ flex: 1, height: 26, borderRadius: 4, background: i === 3 ? "var(--ink)" : "var(--bg-alt)", border: "1px solid var(--line)" }} />
          ))}
        </div>
      </WindowFrame>
      <WindowFrame title="agente · soporte" style={{ bottom: 0, right: 0, width: "60%", height: "22%" }}>
        <Row>
          <span className="mock-pill">✓ resuelto</span>
          <div style={{ flex: 1, fontSize: 11 }}>Cambio talla — Lina V.</div>
          <span style={{ fontFamily: "var(--font-mono)", fontSize: 10, color: "var(--mute)" }}>3s</span>
        </Row>
      </WindowFrame>
    </div>
  );
  ...
  return (
    <div className="mock">
      <WindowFrame title="binah.app / pedidos" style={{ top: 0, left: 0, right: 0, height: "55%" }}>
        {orders.map((o, i) => (
          <Row key={i} highlight={o.hl}>
            ...
            <span className="mock-pill" style={o.hl ? { background: "#fff", color: "#000", borderColor: "#fff" } : undefined}>{o.status}</span>
          </Row>
        ))}
      </WindowFrame>
      ...
      <WindowFrame title="transportadoras" style={{ bottom: 0, right: 0, width: "48%", top: "60%" }}>
        <Row>
          <span className="mock-pill ink">SERVIENTREGA</span>
          <span style={{ flex: 1 }} />
          <span className="mono" style={{ fontSize: 11 }}>48</span>
        </Row>
        <Row>
          <span className="mock-pill">COORDINADORA</span>
          <span style={{ flex: 1 }} />
          <span className="mono" style={{ fontSize: 11 }}>21</span>
        </Row>
      </WindowFrame>
    </div>
  );
  ...
  return (
    <div className="mock">
      ...
      <WindowFrame title="proveedores" style={{ bottom: 0, left: 0, width: "48%", height: "36%" }}>
        <Row><div className="mock-avatar" /><span style={{ flex: 1, fontSize: 12 }}>Textiles Medellín</span><span className="mono" style={{ fontSize: 11 }}>$4.2M</span></Row>
        <Row highlight><div className="mock-avatar ink" /><span style={{ flex: 1, fontSize: 12 }}>Cartón & Co.</span><span className="mono" style={{ fontSize: 11 }}>$1.1M</span></Row>
      </WindowFrame>
    ...
    <div className="mock">
      <WindowFrame title="#operaciones" style={{ top: 0, left: 0, right: "34%", bottom: 0 }}>
        <Row>
          <div className="mock-avatar" />
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 11, fontFamily: "var(--font-mono)", color: "var(--mute)" }}>DANIELA · 14:02</div>
            <div style={{ fontSize: 12 }}>Ya salió el pedido #4821 ✓</div>
          </div>
        </Row>
        <Row highlight>
          <div className="mock-avatar ink" />
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 11, fontFamily: "var(--font-mono)", opacity: 0.7 }}>CARLOS · 14:04</div>
            <div style={{ fontSize: 12 }}>Perfecto, aviso al cliente</div>
          </div>
        </Row>
        <Row>
          <div className="mock-avatar" />
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 11, fontFamily: "var(--font-mono)", color: "var(--mute)" }}>VALERIA · 14:07</div>
            <div style={{ fontSize: 12 }}>Inventario actualizado</div>
          </div>
        </Row>
      </WindowFrame>
      ...
      <WindowFrame title="agenda · martes 23" style={{ top: 0, left: 0, right: 0, bottom: "42%" }}>
        {["09:00", "10:00", "11:00", "12:00"].map((t, i) => (
          ...
          >
            ...
            {i === 1 ? (
              <div style={{ flex: 1, background: "var(--ink)", color: "var(--bg)", borderRadius: 4, padding: "6px 10px", fontSize: 12 }}>Corte de cabello · M. Castro</div>
            ) : i === 2 ? (
              <div style={{ flex: 1, background: "var(--bg-alt)", border: "1px solid var(--line)", borderRadius: 4, padding: "6px 10px", fontSize: 12 }}>Manicure · L. Torres</div>
            ) : (
              <div style={{ flex: 1, height: 8, borderRadius: 3, background: "var(--bg-alt)", opacity: 0.4 }} />
            )}
          </div>
        ))}
      </WindowFrame>
      <WindowFrame title="recursos" style={{ bottom: 0, left: 0, right: 0, height: "38%" }}>
        <Row><div className="mock-avatar" /><span style={{ flex: 1, fontSize: 12 }}>Estilista A</span><Bar w={80} style={{ maxWidth: 120 }} /></Row>
        <Row><div className="mock-avatar" /><span style={{ flex: 1, fontSize: 12 }}>Estilista B</span><Bar w={45} style={{ maxWidth: 120 }} /></Row>
      </WindowFrame>
    </div>
  );
}
...
function MockTasks() {
  ...
  return (
    <div className="mock" style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
      {cols.map((c, i) => (
        <div key={i} style={{ background: "var(--bg)", border: "1px solid var(--line)", borderRadius: 6, padding: 12, display: "flex", flexDirection: "column", gap: 8 }}>
          <div className="mono" style={{ fontSize: 10, color: "var(--mute)", letterSpacing: "0.08em" }}>{c.name.toUpperCase()}</div>
          {c.items.map((it, j) => (
            <div
              key={j}
              style={{
                background: c.highlight && j === 0 ? "var(--ink)" : "var(--bg-alt)",
                color: c.highlight && j === 0 ? "var(--bg)" : "var(--ink)",
                border: "1px solid " + (c.highlight && j === 0 ? "var(--ink)" : "var(--line)"),
                borderRadius: 4,
                padding: "8px 10px",
                fontSize: 12,
              }}
  ...
  return (
    <div className="mock">
      <WindowFrame title="link de pago · WhatsApp" style={{ top: 0, left: 0, width: "50%", bottom: 0 }}>
        <div style={{ padding: 8, background: "var(--bg-alt)", borderRadius: 4, fontSize: 12 }}>
      ...
      <WindowFrame title="métodos" style={{ top: 0, right: 0, width: "46%", height: "48%" }}>
        <Row><span className="mock-pill">PSE</span><span style={{ flex: 1 }} /><span className="mono" style={{ fontSize: 11 }}>42%</span></Row>
        <Row highlight><span className="mock-pill" style={{ background: "#fff", color: "#000", borderColor: "#fff" }}>NEQUI</span><span style={{ flex: 1 }} /><span className="mono" style={{ fontSize: 11 }}>31%</span></Row>
      </WindowFrame>
    ...
    <div className="mock">
      <WindowFrame title="dashboard · vista general" style={{ top: 0, left: 0, right: 0, bottom: 0 }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
          <div style={{ border: "1px solid var(--line)", borderRadius: 4, padding: 12 }}>
            ...
            <div style={{ height: 40, display: "flex", gap: 2, alignItems: "flex-end", marginTop: 6 }}>
              {[20, 35, 28, 48, 40, 55, 70, 60, 65, 80].map((h, i) => (
                <div key={i} style={{ flex: 1, height: h, background: "var(--ink)", opacity: i / 10 + 0.3 }} />
              ))}
            </div>
          </div>
          <div style={{ border: "1px solid var(--line)", borderRadius: 4, padding: 12 }}>
